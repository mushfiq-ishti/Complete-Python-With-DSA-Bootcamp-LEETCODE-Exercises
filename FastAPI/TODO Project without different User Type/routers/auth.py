...
def login_for_acess_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], db: db_dependency):
    ...
    token=create_access_token(user.username, user.id, expires_delta=timedelta(minutes=20)) #creating a JWT token for the authenticated user with an expiration time of 30 minutes
